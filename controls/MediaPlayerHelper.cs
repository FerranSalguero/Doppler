...
{
    public static class WMPHelper
    {
        private static readonly log4net.ILog log = log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

        public static int CleanWMPPlaylistRating(DownloadItem item, IWMPPlaylist playlist, WindowsMediaPlayer player)
        {
            int tracksremoved = 0;
            try
            {
                    ...
                    for (int q = 0; q < tracks.Count; q++)
                    {
                        try
                        {
                            ...
                            if (userRating != "")
                            {
                                ...
                                if (userRatingConvertedValue == feedItem.CleanRating)
                                {
                                    playlist.removeItem(track);

                                    //string trackName = track.name;
                                    //int trackID = track.trackID;
                                    if (Settings.Default.LogLevel > 1) log.Info(String.Format("Track {0} has been deleted based on its WMP rating.", track.name));
        ...
        public static int CleanWMPPlaylistByPlayed(DownloadItem item, IWMPPlaylist playlist, WindowsMediaPlayer player)
        {
            int tracksremoved = 0;
            try
            {
